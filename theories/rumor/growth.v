...
(** Exponential-moment form of "a round is good with constant probability":
    [E[(1/2) ^ goodRound] <= 15/16] for a nonempty informed set, i.e. a round
    is good with probability at least [1/8]. The exponent is the boolean
    [goodRound I r] coerced to [nat]. (Lean: [avg_half_pow_good], where it
    reads [if goodRound I r then 1 else 0].) *)
...
(** Exponential moment for adaptive Bernoulli trials. (Lean:
    [expList_half_pow_goodCount].) *)
...
(** Starting from one informed node, after [k1] rounds the informed set is still
    [<= n/2] with probability at most [2^L (15/16)^k1], whenever [n <= (9/8)^L].
    (Lean: [phase1].) *)
